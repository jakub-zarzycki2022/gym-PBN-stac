...
class PBCN(PBN):
    def __init__(self, PBN_data: PBN_DATA = [], logic_func_data: LOGIC_FUNC_DATA = None):
        ...
        self.control_nodes = np.array([Node(*node_data) for node_data in control_nodes], dtype=object)
        self.control_state = np.empty((self.M), dtype=bool)

    def apply_control(self, control: Union[List[Union[int, bool]]]):
        if len(control) != len(self.control_nodes):
            raise ValueError(f"Control for {len(control)} control nodes provided, when there are {len(self.control_nodes)} in the network.")
    ...
    def step(self):
        ...
        self.state = np.array([node.compute_next_value(combined_state) for node in self.nodes], dtype=bool)
